...
Because of this abstraction, the orchestrator can confidently route a complex reasoning task to `claude-3-5-sonnet`, pass the background memory extraction to `gpt-4o-mini`, and generate embeddings with `text-embedding-3-small`—all while maintaining perfectly unified tracing and telemetry.